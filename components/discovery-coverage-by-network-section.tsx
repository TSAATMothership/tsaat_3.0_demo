...
import { type DiscoveryCoverageByNetworkRow } from "@/lib/discovery-coverage-by-network-rows";
              ...
              <article key={row.networkId} className="panel-alt border border-sky-400/20 p-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    {row.description ? <p className="mt-1 text-xs text-slate-300/75">{row.description}</p> : null}
                    <Link
                      href={drillDownHref}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <div className="flex max-w-full flex-col items-start gap-2 sm:items-end">
                    <div className="flex flex-wrap justify-start gap-2 sm:justify-end">
                      ...
                      {typeof row.overallCoveragePercent === "number" ? (
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2 lg:grid-cols-4">
                  <div className="rounded-md border border-sky-400/15 bg-slate-950/55 px-3 py-2">
                    ...
                    {row.owner ? <p className="mt-1 text-sm text-slate-100">{row.owner}</p> : null}
                  </div>
                  <div className="rounded-md border border-sky-400/15 bg-slate-950/55 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-[0.14em] text-slate-300/70">ATO</p>
                    {row.atoNumber ? <p className="mt-1 text-sm font-semibold text-slate-100">{row.atoNumber}</p> : null}
                  </div>
                  <div className="rounded-md border border-sky-400/15 bg-slate-950/55 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-[0.14em] text-slate-300/70">DIIS</p>
                    {row.diisUrl && row.diisId ? (
                      <Link
                        href={row.diisUrl}

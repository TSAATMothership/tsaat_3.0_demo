...
export interface NetworkActionThroughputPoint {
  weekLabel: string;
  openedCount: number;
  closedCount: number;
  netChange: number;
...
export interface NetworkActionAgeBucketRow {
  bucketLabel: string;
...
export interface NetworkActionOldestFindingRow {
  findingId: string;
  title: string;
  severity: FindingSeverity;
  spiLabel: string;
...
export interface NetworkActionQuickWinRow {
  actionText: string;
  criticalExposureCount: number;
  highRiskCount: number;
  otherCount: number;
...
function ModellingBulletTile({
  ...
  return (
    <article className="panel-alt border-amber-300/35 bg-amber-500/10 p-3 text-amber-100">
      <p className="text-[11px] uppercase tracking-[0.15em] text-slate-200/90">Networks not modelled</p>
      <p className="mt-1.5 text-2xl font-semibold">{notModelledPercent}%</p>
      <p className="mt-0.5 text-xs text-slate-300/90">
        {notModelledCount} of {totalCount} networks
      </p>
...
function RemediationThroughputChart({ rows }: { rows: NetworkActionThroughputPoint[] }) {
...
function FindingAgingBucketsChart({ rows }: { rows: NetworkActionAgeBucketRow[] }) {
  return (
    <section className="panel flex min-h-0 flex-col p-3">
...
function OldestOpenFindingsTable({ rows }: { rows: NetworkActionOldestFindingRow[] }) {
  return (
    <section className="panel flex h-full min-h-0 flex-col p-3">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.findingId} className="border-t border-sky-300/10">
                ...
                <td className="px-3 py-2 text-slate-200">{row.networkName}</td>
...
function ActionQuickWinsTable({ rows }: { rows: NetworkActionQuickWinRow[] }) {
  return (
    <section className="panel flex h-full min-h-0 flex-col p-3">
        ...
        <table className="min-w-full text-sm">
          <thead className="sticky top-0 z-[1] bg-slate-900/95 text-xs uppercase tracking-[0.12em] text-slate-300/80">
            <tr>
              ...
              <th className="px-3 py-2 text-right">Networks</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.actionText} className="border-t border-sky-300/10">
                ...
                <td className="px-3 py-2 text-right text-slate-200">{row.networkCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="relative">
          <div className="flex flex-wrap items-end justify-between gap-2">
            ...
            <span className="rounded-full border border-sky-300/30 bg-slate-900/70 px-2 py-1 text-[11px] text-slate-200">
              Network-Scoped Operational Briefing
            </span>
          </div>
...
export function NetworksActionPanel({
  actionPlan,
  actionThroughput,
  actionAgeBuckets,
  ...
  actionThroughput: NetworkActionThroughputPoint[];
  actionAgeBuckets: NetworkActionAgeBucketRow[];
  actionOldestOpenFindings: NetworkActionOldestFindingRow[];
  actionQuickWins: NetworkActionQuickWinRow[];
}) {
  return (

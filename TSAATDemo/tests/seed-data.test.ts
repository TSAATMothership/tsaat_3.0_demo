...
function referenceOrdinal(value: number): string {
  return String(value).padStart(3, "0");
}

function networkReferenceOrdinal(id: string): string {
  if (id === "net-unassigned") {
    return "000";
  }
  if (id === DISABLED_REFERENCE_NETWORK_ID) {
    return "013";
  }

  const generatedMatch = /^net-new-(\d+)$/.exec(id);
  if (generatedMatch) {
    return referenceOrdinal(Number(generatedMatch[1]) + 5);
  }

  const originalMatch = /^net-(\d+)$/.exec(id);
  if (originalMatch) {
    return referenceOrdinal(Number(originalMatch[1]));
  }

  throw new Error(`Unexpected managed network identifier: ${id}`);
}

function systemReferenceOrdinal(id: string): string {
  const match = /^sys-(\d+)$/.exec(id);
  if (!match) {
    throw new Error(`Unexpected ICT system identifier: ${id}`);
  }
  return referenceOrdinal(Number(match[1]));
}
...
describe("seed dataset", () => {
  ...
  it("stays within required dataset ranges", () => {
    expect(dataset.managedNetworks).toHaveLength(14);
    ...
    for (const network of dataset.managedNetworks) {
      const expectedReferenceOrdinal = networkReferenceOrdinal(network.id);
  ...
  it("contains production and non-production environments for each ICT system", () => {
    ...
    for (const system of dataset.ictSystems) {
      const expectedReferenceOrdinal = systemReferenceOrdinal(system.id);
  ...
  it("carries the disabled reference network through generated snapshots", () => {
    ...
    for (const snapshotFile of snapshotFiles) {
      ...
      const expectedReferenceOrdinal = networkReferenceOrdinal(DISABLED_REFERENCE_NETWORK_ID);

...
describe("Cyber COP ICT System Impact Analyser source wiring", () => {
  ...
  it("keeps asset CMDB record URL wired through demo JSON and analyser row artefacts", () => {
    ...
    expect(dataLoader).toContain('const currentPath = path.join(resolveDataDirectory(), "current.json")');
    expect(dataLoader).toContain("readJsonFile<Dataset>(filePath)");
    expect(dataLoader).not.toMatch(/from ["']@\/lib\/sql-server["']/);
    expect(dataset.assets.length).toBeGreaterThan(0);
    expect(dataset.assets.every((asset) => "cmdbRecordUrl" in asset)).toBe(true);
    expect(
      dataset.assets.every(
        (asset) => asset.cmdbRecordUrl === null || typeof asset.cmdbRecordUrl === "string"
      )
    ).toBe(true);

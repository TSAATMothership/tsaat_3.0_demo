...
async function main() {
  ...
  const current = JSON.parse(await fs.readFile(path.join(repoRoot, "data", "current.json"), "utf8"));
  ...
  const outputDir = path.join(repoRoot, "artifacts", "page-pdf-export-a4-hires");
  ...
  try {
    ...
    const mergedPath = path.join(repoRoot, "artifacts", "DCT-WebApp-All-Pages-A4-HiRes.pdf");
    await mergePdfs(generated, mergedPath);
    console.log(`Merged A4 hi-res PDF created: ${mergedPath}`);
  } finally {

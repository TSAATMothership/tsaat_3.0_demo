...
- **What it does:** adds a circular `C` action at the top-right of the selected server node while preserving the existing bottom-right `D` CMDB drill-through action, and presents the selected server's cyber and discovery compliance in a fixed-size responsive dialog.
- **User perspective:** the user selects a server, selects `C`, and reviews a centred, closeable Server Compliance dialog containing the `Compliance Overview` and `Discovery Compliance` tabs. The Compliance Overview uses the same score-tile visual language as the network and ICT system drill-throughs. Its `Security Posture Indicator breakdown` uses the same score, status-mix, impacted-asset, top-reason, and selectable-row pattern as the ICT system Compliance Overview; selecting an SPI opens a nested Findings and Evidence detail. The Discovery Compliance tab uses the same resized dialog footprint and redesigns tool coverage to use the available space. The dialog can be dismissed with Close or Escape without changing the selected diagram node; `D` continues to open the server's CMDB details.
- **System behaviour:** compliance data is not requested until `C` is selected. The no-store `/api/cyber-cop/impact-analyser-2/compliance` endpoint is scoped by the selected server asset ID and active snapshot date, rejects missing or non-server assets, and returns that server's SPI compliance, finding evidence, and applicable discovery-tool coverage model. The dialog has an explicit responsive width and height, so switching tabs does not resize it; tab content and the SPI table scroll within the fixed workspace. When SPI detail is open, Escape closes that nested detail first; a subsequent Escape closes the parent Server Compliance dialog. The dialog exposes loading, failure, and retry states and cancels obsolete requests when it closes or its scope changes.
- **Outcome:** analysts can assess a server's Security Posture Indicators, findings, evidence, and discovery compliance in context without leaving or resetting the impact diagram.
...
| Cyber COP | Selected Server Compliance view | Contextual compliance assessment for a selected server | Select a server node, select its top-right `C` action, then optionally select an SPI breakdown row | Lazily opens a fixed responsive dialog, requests the selected server's snapshot-scoped compliance model, and opens nested Findings and Evidence for the selected SPI | server asset ID, active data date, SPI definitions, finding evidence, discovery-tool settings | drill-through-style score tiles, `Security Posture Indicator breakdown`, nested SPI detail, and redesigned `Discovery Compliance` tool coverage | `C` is available for selected server nodes; both tabs retain identical outer dimensions; the bottom-right `D` action retains its CMDB drill-through behaviour | missing asset IDs fail with 400; unknown or non-server assets fail with 404; loading failures can be retried | `ServerComplianceView`, shared overview score tile, `buildServerComplianceModel`, compliance API route | Server cyber and discovery compliance can be assessed without leaving the analyser | data is fetched no-store only after `C` is selected; Escape closes nested SPI detail before the parent dialog |

...
- **User perspective:** the user selects one or more scoped ICT systems by selecting either the checkbox or anywhere on the ICT system option row, then explicitly runs the analyser from its own full-size tab. On the resulting risk diagram, a selected server exposes top-right `C` for Compliance View and bottom-right `D` for CMDB Drill Through.
- **System behaviour:** selecting an ICT system option row ticks or unticks its checkbox without closing the selector prematurely; native checkbox and keyboard operation remain available. The analyser starts with no systems selected, does not request analyser data until Run is selected, and scopes its data and SPI drill-through requests to the applied ICT system selection. In this ICT System risk analyser, `C` is server-only; `D` remains available under the existing asset-detail eligibility rules. The former shared impact-analyser `F`/CI-focus action is not rendered and its top-right action position is used by `C` where eligible.
...
- **What it does:** adds an `ICT System Dependencies` control at the far right of the Cyber COP ICT System Impact Analyser filters and opens an in-diagram overlay without replacing the applied Run scope; the ICT-system drill-through reuses the same overlay contract.
- **User perspective:** after the analyser is ready, the user opens a dependency view with the columns `ICT System`, `Environment`, `Server`, `Dependent Server`, `Dependent Environment`, and `Dependent ICT System`. Selected nodes in both `Server` columns expose `C` for Compliance View and retain `D` for CMDB Drill Through. The user can filter by one or more `Dependent ICT Systems`, select `Export Excel` for the currently filtered six-column data, then select Close or press Escape to return to the unchanged impact diagram.
- **System behaviour:** opening the dependency view hides the parent `ICT System Impact Analyser Diagram` filter row while retaining its state; closing the view restores that row in both Cyber COP and ICT-system drill-through contexts. Dependency data loads only when the view opens and uses real directed server-to-server CI dependency records whose source server belongs to an applied ICT system model. The dependent ICT system filter is applied worker-side, including exact `Not Modelled` selection, and the Excel rows use the same accepted worker-filtered row indexes as the diagram. The dependent endpoint retains its actual model and environment; targets outside a modelled ICT system environment terminate at one red circular `Not Modelled` node placed first in the final column. `C` is available on real selected server nodes in both `Server` and `Dependent Server`, not on `Not Modelled`; the former shared `F`/CI-focus action is not rendered. Self-links, dangling endpoints, non-server relationships, and duplicate dependency IDs are excluded.
- **Outcome:** cross-system and unmodelled server dependencies can be traced from either ICT System analyser without losing its applied scope.

### Feature: Selected Asset Compliance View
- **What it does:** uses circular top-right `C` to present the selected asset's cyber and discovery compliance in a fixed-size responsive dialog while preserving bottom-right `D` for CMDB Drill Through.
- **User perspective:** the user selects an eligible node, selects `C`, and reviews a centred, closeable Compliance View containing `Compliance Overview` and `Discovery Compliance`. The Compliance Overview uses the score-tile visual language of the network and ICT system drill-throughs. Its `Security Posture Indicator breakdown` uses the same score, status-mix, impacted-asset, top-reason, and selectable-row pattern as the ICT system Compliance Overview; selecting an SPI opens nested Findings and Evidence. The Discovery Compliance tab uses the same fixed dialog footprint. Close or Escape dismisses the view without changing the selected diagram node; `D` continues to open CMDB details.
- **System behaviour:** `C` eligibility is explicit by analyser context: server nodes only in ICT System risk analysers; real server nodes in both `Server` and `Dependent Server` columns in ICT System Dependencies; and every canonical asset type (`server`, `workstation`, `network-device`, `storage-device`, `printer-device`, `other`) in Cyber COP and drill-through Network Impact Analysers. The former shared impact-analyser `F`/CI-focus action has been removed and replaced by `C` where the context permits compliance. Compliance data is requested only after `C` is selected. The no-store `/api/cyber-cop/impact-analyser-2/compliance` endpoint is scoped by asset ID and active snapshot date, rejects a missing ID with 400 and an asset absent from the selected snapshot with 404, and returns SPI compliance, finding evidence, and discovery-tool coverage applicable to the selected asset type. Switching tabs does not resize the dialog; when SPI detail is open, Escape closes that detail before the parent Compliance View.
- **Outcome:** analysts can assess an eligible asset's Security Posture Indicators, findings, evidence, and discovery compliance without leaving or resetting the diagram.
...
- **User perspective:** the user selects one or more real networks by selecting either the checkbox or anywhere on the network option row, then explicitly selects Run to build the aggregate network report. Any selected asset node can open Compliance View with `C` or CMDB Drill Through with `D`.
- **System behaviour:** selecting a network option row ticks or unticks its checkbox without closing the selector prematurely; native checkbox and keyboard operation remain available. Pending selection is local and is not written to the URL; the analyser starts with no selection, Run remains disabled until a network is selected, and changing or clearing the pending selection removes the previously applied report. On Run, each selected network uses the same topology, declared-asset, direct-asset, finding, Network axis, Assets axis, Asset Type filter, Asset Details, SPI findings, and all-asset `C` eligibility as the single-network drill-through. Overlapping model assets are de-duplicated while their real network names are retained. The shared analyser does not render the former `F`/CI-focus action.
...
| Cyber COP | ICT System Impact Analyser tab | Run-gated selected-system impact analysis | Open the main analyser tab, select an ICT system option row or checkbox, then select Run | Ticks the corresponding checkbox from either interaction and mounts the analyser only after Run with the applied ICT system IDs | current Impact ICT system scope, analyser selection, open findings | Selected-system analyser diagram and SPI drill-throughs | defaults to no ICT system selection; Run requires at least one selection; selected servers expose `C`; other asset types do not | unavailable systems are removed when the current Impact scope changes; changing or clearing the pending selection removes the previous diagram | `CyberCopDashboard`, impact analyser API | Intentional full-size ICT system impact analysis | `D` remains; the shared `F`/CI-focus action is removed |
| Cyber COP | ICT System Dependencies | Directed dependency overlay for applied ICT systems | Select `ICT System Dependencies`, filter `Dependent ICT Systems`, optionally select `Export Excel`, then Close | Hides and later restores the parent diagram filters; renders the six dependency columns over the diagram and exports accepted filtered rows | applied ICT system IDs, server dependencies, dependent ICT system selection | Dependency diagram and Excel-readable `.xls` data | both `Server` and `Dependent Server` selected nodes expose `C` and retain `D`; `Not Modelled` does not | non-server, dangling, self, and duplicate dependency records are excluded | dependency API, shared impact analyser, workbook exporter | Trace modelled and unmodelled dependencies without losing Run scope | same interaction contract is reused in the ICT-system drill-through |
| Cyber COP | Selected Asset Compliance view | Contextual compliance assessment for an eligible selected asset | Select an eligible node, select top-right `C`, then optionally select an SPI breakdown row | Lazily opens a fixed responsive dialog, requests the asset's snapshot-scoped compliance model, and opens nested Findings and Evidence for the selected SPI | asset ID, active data date, SPI definitions, finding evidence, discovery-tool settings | drill-through-style score tiles, `Security Posture Indicator breakdown`, nested SPI detail, and `Discovery Compliance` tool coverage | eligibility is server-only for ICT risk, both server columns for dependencies, and all six asset types for Network Impact; both tabs retain identical outer dimensions; bottom-right `D` retains CMDB behaviour | missing asset IDs fail with 400; assets absent from the snapshot fail with 404; loading failures can be retried | Compliance View, shared overview score tile, `buildAssetComplianceModel`, compliance API route | Asset cyber and discovery compliance can be assessed without leaving the analyser | data is fetched no-store only after `C`; the former shared `F`/CI-focus action is removed |
| Cyber COP | Network Impact Analyser tab | Run-gated selected-network impact analysis | Open the main analyser tab, select a network option row or checkbox, then select Run | Ticks the corresponding checkbox, unions and de-duplicates the selected network topology models, then mounts the analyser with the applied network IDs | current real-network scope, analyser selection, topology and asset relationships, open findings | Aggregate network analyser diagram, Asset Details, SPI drill-throughs, and Compliance View | defaults to no network selection; Run requires at least one selection; selection alone never loads data; `C` applies to every canonical asset type | invalid or out-of-scope network IDs are excluded; changing or clearing selection removes the previous diagram; an empty applied scope fails closed | `CyberCopDashboard`, shared impact analyser and Cyber COP analyser API | Full-size multi-network impact analysis with single-network drill-through semantics | `D` remains and the shared `F`/CI-focus action is removed; pending and applied selection are local state |

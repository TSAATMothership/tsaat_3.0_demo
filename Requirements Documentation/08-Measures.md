...
| Bulk KPI scope evaluation | performance report KPI matrix rows | `tsaat.usp_evaluate_kpi_snapshot_bulk` accepts multiple scope rows and returns KPI rows grouped by scope key | KPI metadata tables plus scoped asset/system/network/finding JSON | Runtime SQL result | SQL Server | avoids one application SQL process call per matrix row |
...
- Derived page models are cached for a short operational window and invalidated when measures or discovery settings change.

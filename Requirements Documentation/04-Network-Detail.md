...
- **System behaviour:** `networkDetailTab` in the query string controls the main tab; the topology view is a client-side modal fed by runtime topology data built from real network relationships and CI dependencies, with `net-unassigned` excluded from network model nodes. In its Network Impact Analyser, top-right `C` opens Compliance View for every canonical asset type and bottom-right `D` opens CMDB Drill Through. Related Asset names, discovery coverage names, asset inventory names, affected-CI names, and device identifiers also open the shared read-only CMDB Drill Through by stable asset ID.
- **Outcome:** tab states are bookmarkable; topology is not.

### Feature: Network Impact Analyser Asset Actions
- **What it does:** provides contextual Compliance View and CMDB actions on selected asset nodes in the drill-through Network Impact Analyser.
- **User perspective:** after selecting any server, workstation, network device, storage device, printer device, or other asset, the user selects `C` for `Compliance Overview` and `Discovery Compliance`, or `D` for CMDB Drill Through.
- **System behaviour:** `C` is available for all six canonical asset types and lazily requests the selected asset and snapshot from `/api/cyber-cop/impact-analyser-2/compliance`; applicable discovery tools are selected from the asset type's configured scope. `D` retains its existing bottom-right behaviour. The former shared impact-analyser `F`/CI-focus action and eligibility badge are not rendered; `C` occupies the top-right action position where eligible.
- **Outcome:** all network assets can be assessed without leaving or resetting the drill-through analyser.
...
| Network Detail | Network Impact Analyser asset actions | Compliance and CMDB drill-through for selected assets | Select any asset node, then select `C` or `D` | Opens the fixed-size Compliance View lazily for `C`, or the shared CMDB Drill Through for `D` | selected asset ID and type, `dataDate`, SPI findings, discovery settings | `Compliance Overview`, `Discovery Compliance`, or CMDB details | `C` supports all six canonical asset types; `D` remains; shared `F`/CI-focus is removed | missing asset ID fails with 400; asset absent from snapshot fails with 404 | shared impact analyser, compliance API, CMDB Drill Through | In-context asset assessment | compliance applicability follows discovery tool asset-type scope |
...
| Network Detail | Network hierarchy and topology | `tsaat` | `managed_network_hierarchy`, `ict_system_hierarchy`, `network_declared_system`, `network_declared_asset`, `ci_dependency` | parent-child keys and dependency fields | string, enum-like | topology modal and Network Impact Analyser relationship context | Read | combined into topology graph | selected asset nodes use top-right `C` for Compliance View and retain bottom-right `D`; no shared `F`/CI-focus eligibility action is produced | runtime graph build | topology includes synthetic relation edges |
...
- `DetailedTopologyView` creates runtime graph layouts and client-only interactions from already-loaded topology data. Within the shared Network Impact Analyser, `C` replaces the former `F`/CI-focus action for eligible selected assets, `D` remains, and the Asset Details slide-out is a read-only renderer over the selected asset and relationship rows, including optional asset `cmdb_record_url`.

...
## 1. Page Overview
- **Page name:** Settings
- **Purpose:** provide application-level configuration controls for database connectivity and authentication password lifecycle.
- **User outcome:** the user can validate and save database connectivity/SSL settings, and can change the current application password from the dedicated password settings tab.
...
- password changes are written to `tsaat.app_user` as salted PBKDF2-HMAC-SHA256 hash material and session version is incremented.
...
- **System behaviour:** `/api/settings/password` validates current credentials, writes a new salted hash + salt + iteration metadata into `tsaat.app_user`, increments `session_version`, and clears the session cookie.
...
| Settings | Password settings | Change current app password | Enter current/new/confirm password then click `Save Password` | Validates current password and persists new salted hash metadata to `tsaat.app_user`; increments session version and clears session cookie | current password, new password, confirm password | password update status and re-login requirement | only authenticated session can change password; current password must match; new/confirm must match | new password length and current-password verification are enforced server-side | `/api/settings/password`, `tsaat.app_user` | Password rotation without plaintext storage | success forces sign-out |
...
## 5. Database Mapping
This page is mixed file-backed and database-backed. Database connection settings are file-backed (`DB_config`), while password settings are stored in SQL Server (`tsaat.app_user`).

Primary dependencies:

- local configuration file `DB_config`
- SQL Server schema `tsaat` for validation checks
- SQL Server table `tsaat.app_user` for password updates
...
| Settings | Password settings storage | `tsaat` | `app_user` | `username`, `password_hash`, `password_salt`, `hash_algorithm`, `iteration_count`, `session_version`, `is_active`, `password_changed_at_utc` | mixed | stores application login credentials and invalidation metadata | Read and Update | lookup by username from authenticated session | `session_version` increments on each password change | PBKDF2-HMAC-SHA256 salted hash verification + rewrite | plaintext password is never persisted or returned |
...
| Password change | rotate app login secret | verify current password hash, persist new salted hash, increment `session_version`, clear cookie | `tsaat.app_user` credential fields + authenticated session cookie | Runtime | API/backend | old sessions become invalid after update |
...
- Password writes are one-way salted hashes in `tsaat.app_user`; plaintext is not persisted.
- Successful password changes increment `session_version` and force re-authentication.

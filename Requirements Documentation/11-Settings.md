...
| Settings | Schema validation - required columns | `tsaat` | selected required tables | `snapshot_date`, `network_id`, network platform/reference columns, `system_id`, system platform/reference columns, `asset_id`, `asset_type`, `finding_id`, `spi_id`, `workflow_status`, `dependency_id`, `tool_id`, `severity` | mixed | confirms minimum structural contract | Read | validation checks column existence by table | all required columns must exist, including network/system ATO, DIIS, and APM reference columns | SQL validation query | exact list maintained in code |

...
- **User perspective:** the user sees discovery-enabled status, target-state availability, per-asset-type target/discovered totals, and charted target-vs-discovered comparisons by network.
- **System behaviour:** discovered assets are matched to target-state assets using trim+lowercase name matching plus exact asset type; summary cells render grouped bars when both target and discovered counts are present, otherwise render `Target State Missing` or `Discovery Missing`. The filter container shows a right-aligned `Total Network` count based on the currently filtered network-discovery rows.
...
| Discovery | Network discovery summary | Compare DB-backed target state and matched discovered totals by network and asset type | Open tab, apply filters | Resolves target-state names from DB, matches discovered assets by normalized name + asset type, and renders per-cell totals + grouped bar chart or missing-state message | networks, scoped assets, network target-state assets | Network summary table with `Target State Provided` and per-asset-type target/discovered visuals | target-state ignores system and environment filters | name normalization is trim+lowercase; matched count is 1:1 by name+type | `NetworkDiscoverySummaryTableClient`, target-state matching helper | Network discovery posture view | chart is hidden on missing target/discovery states |
...
| Target-state provided | derive network-level target-state availability | `Yes` when any target-state rows exist for the network, otherwise `No` | `network_target_state_asset` grouped by network | Runtime | backend | independent of discovered asset presence |

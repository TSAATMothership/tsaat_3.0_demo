...
- **System behaviour:** the page builds in-scope asset rows and consumes SQL-produced compliant, non-compliant, and tool coverage counts. The coverage-by-tool panel expands into the remaining tab height.
...
| Discovery | Summary tab | Operational discovery dashboard | Open tab | Builds coverage snapshot cards from SQL discovery coverage rows and fills the remaining tab area with coverage-by-tool analysis | scoped assets, discovery settings, discovery coverage rows | Snapshot cards and coverage-by-tool panel | reflects current filtered scope | none beyond scope parsing | `DiscoveryCoverageByToolSection`, `usp_evaluate_discovery_coverage_snapshot` | Discovery gap summary | remediation report launcher removed from this tab |
...
| Discovery | Discovery tool settings | `tsaat` | `discovery_tools_settings_version`, `discovery_tool`, `discovery_tool_asset_scope`, `discovery_coverage_source`, `discovery_tool_detection_definition`, `discovery_tool_detection_rule`, `discovery_tool_detection_rule_value` | version, tool metadata, `tool_id`, asset-type scope flags, source keys, detection keys, rule values | mixed | defines required tools, tool labels, and detection rules | Read and Update for scope; read-only for detection metadata | latest settings version plus child tool rows; SQL rules evaluated by `usp_evaluate_discovery_coverage_snapshot` | `na` marks tool not applicable for an asset type | SQL coverage rules | scope covers `server`, `workstation`, `network-device`, `storage-device`, `printer-device`, `other` |
...
| --- | --- | --- | --- | --- | --- | --- |
| Asset discovery coverage | decide whether an asset is discovery compliant | SQL Server evaluates required tools for the asset type using seeded detection rules; asset is compliant when no required tool is missing | assets, discovery tool settings, discovery coverage rule tables | Runtime SQL result | SQL Server/backend | tools with asset-type scope `na` return null and do not count against compliance; applies consistently across all six asset types |
...
- Discovery compliance depends on the latest saved discovery tool settings and DB-backed discovery detection rules.
...
- Tool settings changes affect subsequent SQL runtime evaluations rather than historical snapshot facts.

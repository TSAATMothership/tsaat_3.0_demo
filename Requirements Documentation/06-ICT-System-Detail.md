...
- `/api/cyber-cop/impact-analyser-2/compliance`
- `/api/cyber-cop/impact-analyser-2/dependencies`
...
- **System behaviour:** `systemDetailTab` in the query string controls the main visible tab; the topology modal is client-side only and receives runtime topology data built from the snapshot model. In the embedded ICT System risk analyser, a selected server exposes top-right `C` for Compliance View and bottom-right `D` for CMDB Drill Through; other asset types do not expose `C`. Related Asset names, discovery coverage names, and all affected-CI device names continue to open the shared read-only CMDB Drill Through by stable asset ID. The former shared impact-analyser `F`/CI-focus action is not rendered.
- **Outcome:** tab states are bookmarkable; topology modal state is not.

### Feature: Embedded ICT System Impact Analyser and Dependencies
- **What it does:** adds `ICT System Dependencies` to the far right of the embedded ICT System Impact Analyser filters and opens the six-column dependency diagram over the current risk diagram.
- **User perspective:** after the embedded analyser is ready, the user opens `ICT System Dependencies`, filters `Dependent ICT Systems`, selects `Export Excel` when required, and closes the overlay to return to the unchanged risk diagram. Both `Server` and `Dependent Server` selected nodes expose `C` for Compliance View and retain `D` for CMDB Drill Through.
- **System behaviour:** opening the overlay hides and retains the parent `ICT System Impact Analyser Diagram` filters; closing it or pressing Escape restores those filters and their values. The columns are `ICT System`, `Environment`, `Server`, `Dependent Server`, `Dependent Environment`, and `Dependent ICT System`. The dependency request is scoped to the drill-through ICT system and loads only when opened. The `Dependent ICT Systems` filter and Excel-readable `.xls` export use the same accepted filtered rows; unmodelled target servers terminate at the red `Not Modelled` node. `C` is available for real nodes in both server columns, not `Not Modelled`; `D` remains and no shared `F`/CI-focus action is rendered.
- **Outcome:** users can trace, filter, export, and assess server dependencies without losing the ICT-system drill-through scope.
...
| ICT System Detail | Embedded impact analyser and dependencies | ICT System risk analysis with a directed dependency overlay | Select a server and use `C` or `D`; select `ICT System Dependencies`, filter `Dependent ICT Systems`, use `Export Excel`, then Close | Keeps parent analyser filters hidden while the overlay is open and restores them unchanged on close; dependency `C` applies in both server columns | drill-through system ID, server dependency rows, selected dependent systems, `dataDate` | Risk Compliance View or CMDB details, dependency diagram, and Excel-readable `.xls` rows | ICT risk `C` is server-only; dependency `C` covers real `Server` and `Dependent Server` nodes; `D` remains; shared `F`/CI-focus is removed | dependency overlay is disabled until the parent analyser is ready; `Not Modelled` has no `C` | shared impact analyser, dependency and compliance APIs, workbook exporter | In-context risk and dependency assessment | Close and Escape restore the parent filter row and focus context |
...
| ICT System Detail | Topology relationships | `tsaat` | `ict_system_hierarchy`, `ci_dependency`, `network_declared_asset`, `network_declared_system` | parent-child keys, dependency endpoints, declared relationship keys | mixed | topology modal, embedded risk analyser, and `ICT System Dependencies` | Read | combined into runtime risk and directed server-dependency graphs | dependency self-links, dangling endpoints, non-server links, and duplicate IDs are excluded; unmodelled targets use `Not Modelled` | runtime topology layout and filtered Excel projection | no shared `F`/CI-focus action; `C` and `D` follow analyser-specific eligibility |
...
- `DetailedTopologyView` creates runtime graph layouts and modal-only interactions from already loaded topology data. Its shared ICT System risk analyser exposes `C` for selected servers only, retains `D`, removes the former `F`/CI-focus action, and uses the Asset Details slide-out as a read-only renderer over selected asset and relationship rows, including optional asset `cmdb_record_url`.
- The `ICT System Dependencies` overlay, its filter visibility state, `Dependent ICT Systems` selection, `Export Excel`, Close, and Escape behaviour are client-only view state over snapshot-scoped dependency rows.

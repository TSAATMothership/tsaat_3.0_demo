...
### Feature: Selected Server Compliance View
- **What it does:** adds a circular `C` action at the top-right of the selected server node while retaining the existing bottom-right `D` action and its CMDB drill-through path, then presents a fixed-size responsive compliance workspace.
- **User perspective:** selecting `C` opens a centred Server Compliance dialog with Close and Escape dismissal and two tabs: `Compliance Overview` and `Discovery Compliance`. Compliance Overview uses the shared network and ICT system drill-through score tile and a `Security Posture Indicator breakdown` table whose rows open nested Findings and Evidence detail. Discovery Compliance uses the identical dialog dimensions and fills the expanded workspace with discovery score, tool-coverage, status, and ownership information. Closing the dialog restores the unchanged analyser state, and selecting `D` still opens CMDB details.
- **System behaviour:** the canvas-selected server coordinates anchor an accessible HTML `C` control above the independent `D` hit target. `ServerComplianceView` is rendered through a body portal with explicit viewport-responsive width and height constraints; tab bodies scroll internally, so changing tabs never changes the outer box. It lazily requests `/api/cyber-cop/impact-analyser-2/compliance?assetId=...&dataDate=...` only while open. The no-store route validates the server, loads the selected snapshot, and uses `buildServerComplianceModel` to combine SPI evaluation status, score contribution, impacted-asset count, reasons, findings and evidence with the discovery tools applicable to servers. The SPI table follows the ICT system breakdown's score-descending then SPI-ID ordering. Escape closes an open SPI detail before it closes the parent dialog; aborted, failed, and retry requests are handled without mutating analyser selection.
- **Outcome:** the analyser adds drill-through-consistent server Security Posture Indicator and discovery detail without coupling dialog lifecycle to diagram rendering or regressing CMDB access.
...
| Cyber COP | Selected Server Compliance view | Contextual compliance assessment for a selected server | Select a server node, select its top-right `C` action, then optionally select an SPI breakdown row | Anchors the HTML action to the selected canvas node, opens a fixed responsive portal dialog, lazily requests the server model, and layers nested SPI Findings and Evidence inside it | server asset ID, active data date, SPI definitions, finding evidence, discovery-tool settings | shared drill-through score tiles, `Security Posture Indicator breakdown`, nested SPI detail, and expanded Discovery Compliance tool coverage | only selected server nodes expose `C`; Compliance and Discovery retain one fixed outer footprint; the bottom-right `D` action remains an independent CMDB drill-through | route requires an asset ID and an asset whose canonical type is server; failed loads expose Retry | `IctSystemImpactAnalyser2Chart`, `ServerComplianceView`, shared overview score tile, compliance API route, `buildServerComplianceModel` | Server cyber and discovery compliance is available without disturbing diagram state | route responses are no-store and dialog fetches are abortable; Escape closes SPI detail before the parent dialog |
...
| Page Name | Feature Name | Schema | Table | Column | Data Type (if known) | Purpose on Page | CRUD Usage | Join / Relationship Logic | Default Value / Rule | Calculation / Transformation | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ICT Systems | System identity | `tsaat` | `ict_system` | `system_id`, `name`, `criticality`, `security_domain`, `modelling_status`, `diis_defined`, `diis_id`, `ato_number`, `apm_number`, detail columns | mixed | row identity, posture context, slideout data | Read | assets and findings join via `system_id` | ATO, DIIS, and APM references are loaded/backfilled; fallback descriptive metadata allowed | direct display and rollups | |
...
| KPI-1 through KPI-10 SQL evaluation | KPI score rows | `tsaat.usp_evaluate_kpi_snapshot` evaluates enabled KPI definitions against the selected scope using DB calculation metadata, scoped SQL SPI evaluations, SQL discovery coverage rows, effective findings JSON, systems, and networks | KPI metadata tables plus snapshot tables and effective findings | Runtime SQL result, stored definition | SQL Server | zero-safe counts; KPI-7/8 use SQL deterministic hash-equivalent proxy logic |
| Bulk KPI scope evaluation | performance report KPI matrix rows | `tsaat.usp_evaluate_kpi_snapshot_bulk` evaluates multiple scoped KPI matrix rows in one SQL call | KPI metadata plus scoped asset/system/network/finding JSON | Runtime SQL result | SQL Server | reduces one-SQL-process-per-row report overhead |
...
- Derived page models use short-lived server memory caching and compact dataset signatures; caches are invalidated by measures and discovery settings changes.
...
| Page Name | Feature Name | Schema | Table | Column | Data Type (if known) | Purpose on Page | CRUD Usage | Join / Relationship Logic | Default Value / Rule | Calculation / Transformation | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Settings | File-backed connection settings | local ignored file | `DB_config` | encrypted JSON envelope (`format`, `version`, `keyProvider`, `algorithm`, `ciphertextBase64`, `updatedAtUtc`) | text/json | source of saved database + SSL settings | Read and Update | no database join; file read/write only | UI missing-file defaults SSL to disabled; offline scripts can create encrypted file from confirmed input or recreate an undecryptable local file; database creation defaults to client-payload seed loading for remote SQL Server support | DPAPI decrypt/encrypt + normalization before save or script confirmation | API never returns plaintext password; SQL mode uses stored-password placeholder token; confirmation helper redacts SQL passwords |

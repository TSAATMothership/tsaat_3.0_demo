...
- **System behaviour:** network detail fields are resolved from DB columns when present, otherwise synthesized from the network ID and name; persisted ATO, DIIS, and APM references are populated during load/migration, and the security accreditation table shows stored `diis_id` or `Missing` without rendering a DIIS link.
...
| Network Detail | Details tab | Metadata and risk charts | Open tab | Resolves metadata, renders persisted accreditation references and stacked risk charts | network detail fields, findings | Ownership and risk context | metadata may fall back when source columns blank; ATO and DIIS display from stored/backfilled values; APM is persisted for network records and available to downstream consumers | none | `resolveNetworkDetailFields()`, risk charts | Narrative network view | support and service links may be synthetic; DIIS is not linked from the Security Accreditation table |
...
- Metadata fields may be synthetic when source columns are blank, except persisted reference fields are loaded/backfilled and Security Accreditation DIIS ID displays `Missing` when blank.
